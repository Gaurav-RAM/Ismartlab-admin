...
export default function CollectorDocumentList() {
  const { id } = useParams(); // optional: use collectorId from route
  ...
  // demo data — swap with API data for collector `id`
  ...
  return (
    <CollectorListUnified
      ...
      rows={pageRows}
      // custom per-row actions are optional; the component already shows View/Download by default.
      renderActions={(d) => (
      ...
      // no bulk actions by default for documents; add if needed
      bulkActions={[]}
      selectedAction=""
      onActionChange={() => {}}
      onApply={() => {}}

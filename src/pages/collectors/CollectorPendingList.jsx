// src/pages/labs/CollectorPendingList.jsx
...
export default function CollectorPendingList() {
  ...
  // equality filters always include pending
  ...
  useEffect(() => {
    ...
    if (!debouncedSearch) {
      const q = fsQuery(colRef, ...equalityWhere(appliedFilters), orderBy('createdAt', 'desc'));
      const unsub = onSnapshot(q, (snap) => {
        ...
        const collectors = uniq(ds.map(d => ({ value: d.collector?.id ?? (d.members?.[0]?.id ?? ''), label: d.collector?.name ?? (d.members?.[0]?.name ?? '') }))).filter(x => x.value);
    ...
    // with search: merge per-field prefix queries (ordered by each field)
    const s = debouncedSearch.toLowerCase();
    const fields = ['customer.nameLower','lab.nameLower','collector.nameLower','testPackageLower'];
  ...
  // bulk actions example (optional: set to pending->active)
  ...
  const handleExport = () => {
    ...
    a.href = url; a.download = `pending_collectors_${Date.now()}.csv`;
    document.body.appendChild(a); a.click(); a.remove();
    URL.revokeObjectURL(url);
  };

  // header
  const breadcrumbs = [{ label: 'Dashboard', to: '/' }, { label: 'Pending Collector List' }];
            ...
            <td>
              <Stack direction="row" spacing={0.5}>
                ...
                <Tooltip title="Delete">
                  <IconButton size="small" color="error" aria-label="delete" onClick={() => alert('Implement delete if needed')}>

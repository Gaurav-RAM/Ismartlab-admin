// CollectorForm.attractive.jsx (DB-only, no Storage) — no focus bg + working Save + navigate
...
// Firestore only (modular v9+)
import { collection, addDoc, serverTimestamp } from 'firebase/firestore';
import { db } from '../../firebase';

// ---------- memoized helpers OUTSIDE to avoid remounts ----------
...
const SectionBody = React.memo(function SectionBody({ children }) {
  return (
    <Paper sx={{ borderTopLeftRadius: 0, borderTopRightRadius: 0, p: 2, bgcolor: 'background.paper' }}>
      {children}
    </Paper>
  );
});
...
const appTheme = createTheme({
  ...
  components: {
    MuiCssBaseline: {
      styleOverrides: {
        // Neutralize Chrome autofill background so it stays transparent
    ...
    MuiTextField: {
      defaultProps: { size: 'small', variant: 'outlined', fullWidth: true, autoComplete: 'off' }
    },
    MuiOutlinedInput: {
      styleOverrides: {
        root: {
    ...
    MuiButton: {
      ...
      styleOverrides: { root: { textTransform: 'none', borderRadius: 10, boxShadow: 'none' } }
    },
    MuiPaper: {
      defaultProps: { variant: 'outlined', elevation: 0 },
      styleOverrides: {
        root: { borderColor: 'var(--mui-palette-divider)', boxShadow: '0 2px 12px rgba(31,42,68,0.06)' }
      }
    }
  }
});
...
export default function CollectorForm() {
  ...
  const [photoDataUrl, setPhotoDataUrl] = React.useState(null); // optional DB-only image
  ...
  // Save handler: writes to Firestore only and navigates
  const handleSave = async () => {
    ...
    try {
      ...
      navigate(LIST_PATH, { replace: true, state: { createdId: docRef.id, toast: 'Collector created' } });
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
        ...
        <Box sx={{ bgcolor: 'background.default', minHeight: '100vh', py: 3 }}>
          <Container maxWidth="xl">
                    ...
                    <Grid item xs={12} md={4}>
                      <Paper sx={{ p: 2, height: '100%' }}>
                        <FormControl fullWidth>
                          ...
                          >
                            {preview ? (
                              <Avatar src={preview} alt="Profile" sx={{ width: 96, height: 96 }} />
                            ) : (
                    ...
                    <Grid item xs={12} md={8}>
                      <FormControl fullWidth>
                        ...
                        <Select
                          ...
                          displayEmpty
                          renderValue={(value) => (value ? value : 'Select lab')}
                          size="small"
                        >
                      ...
                      <Grid item xs={12} md={4}>
                        <FormControl fullWidth>
                          ...
                          <Select
                            ...
                            displayEmpty
                            renderValue={(value) => (value ? value : 'Select state')}
                            size="small"
                            disabled={!country}
                          >
            ...
            <Typography variant="caption" color="text.secondary">
              Kvate: Your Ultimate Entertainment Hub v1.2.5
            </Typography>
          </Container>
        </Box>
